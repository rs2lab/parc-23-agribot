...
class UcvActionPlan:
    ...
    def init(self):
        """Initializes all variables needed to execute the plan."""
        ## NOTE: Must be implemented downstream
        pass

    def has_next_step(self):
        """Returns True if this plan has a next step to be executed."""
        ## NOTE: Should be implemented downstream
        return False
    
    def consume_step(self, fn):
        """Consumes one step of the plan. Receives a function `fn`
        that receives a twist command as a param."""
        fn(self.to_twist())    

    def finish(self):
        """Finishes the plan execution."""
        ## NOTE: Should be implemented downstream
        pass
